...
## D8 — Mercy / сухая победа = exact N:0 (2026-07-21)

**Decision:** При `mercyEnabled` победа «в сухую» только при точном счёте `mercyPoints:0` или `0:mercyPoints`. Отрыв на N очков при ненулевом счёте соперника (5:1, 6:1…) **не** является сухой победой — матч идёт до обычных правил.

**Why:** Продуктовое уточнение: «в сухую» = ни одного очка у соперника.
