...
На экране первого пароля «Выйти» отправляет ровно один logout POST: после
успеха или `401` текущий `/auth/me` возвращает `401`, Back не восстанавливает
доступ, другая действующая сессия того же пользователя остаётся активной.
Известный отказ сохраняет поля, сообщает о нём с фокусом и допускает явный
повтор без одновременного Save/Logout.
...
Экран первого пароля переводит известные причины в действия без внутренних
кодов, неизвестные причины обобщает. Несовпадение связано с обоими полями;
после редактирования устаревшая ошибка исчезает. Каждый пароль можно отдельно
раскрыть и снова скрыть без отправки, потери значения или замены поля. Ошибка
формы получает фокус после каждой явной неуспешной отправки, даже когда текст
ошибки не изменился; редактирование не переносит его повторно, затем
Tab/Shift+Tab остаются обычными.
...
Given отправка ручного матча ожидает ответ, When пользователь пытается изменить
состав, название, формат или правила, Then видимые значения и один отправленный
payload остаются согласованы. Given известный отказ, Then значения доступны для
исправления и новая отправка происходит только после явного действия.
...
Given ручное включение ждёт ответ, Then выбранный игрок остаётся видимым и
неизменяемым. Given известный отказ, Then выбор сохраняется; Given подтверждённый
успех, Then актуальный состав показывается после чтения. Given ошибка построения
сетки, Then она видна внутри открытого Dialog с сохранённым выбором. Given
неизвестный исход, Then GET и Close/reopen не вызывают и не разблокируют второй
POST; успех требует подтверждённый ответ и актуальное чтение сетки.
Given POST подтверждён, но первое чтение сетки отклонено, Then закрытие и
повторное открытие окна не разрешают второй POST; When явный GET подтверждает
актуальную сетку, Then окно закрывается и дальнейшая перестройка вновь доступна
только как отдельное явное действие.
...
Авторизованный пользователь получает на главной все свои доступные текущие
дела игрока, текущего судьи и организатора без дублей: первые два сразу,
остальные через «Ещё N текущих дел». Идущая игра стоит выше более нового
ожидающего матча. Последние пять завершённых standalone-матчей/турниров
сортируются в общем хронологическом порядке; «Только мои» отбирает участие
игроком до ограничения пятью, включая релевантные турниры. Более 50 новых
чужих результатов не вытесняют эти пять и не обнуляют `myStats`.
Более 50 собственных результатов сохраняют точные wins/losses/averagePoints/rival,
при этом полные детали standalone-матча загружаются лишь для текущих и пяти
недавних карточек. Большая активная турнирная сетка сводится по строкам матчей,
участников и действующих судейских сессий без `getMatch` для каждой игры.
Видимый admin каталог чужих активных турниров не становится списком его личных
`currentTasks`; совместимый `activeEvents.tournament` сохраняет `topThree`
одного выбранного видимого турнира без загрузки деталей каждой игры остальных
турниров каталога. Роли и гостевые/зарегистрированные имена остаются точными.
...
При pending/503 первого GET Home все разрешённые входы в историю, рейтинг,
матчи, турниры, команды, помощь, профиль и уведомления остаются достижимыми,
а admin-вход зависит от роли; неизвестные метрики не становятся нулями.
В краткой карточке обычного завершённого матча стороны и счёт идут первыми,
длительность третьей, `winnerSide` отмечен у верной стороны с доступным
текстом даже при одинаковых именах; при отсутствии `winnerSide` победа не
угадывается. Повторной строки победителя и обычного статуса «Завершён» нет;
stopped/cancelled/voided сохраняют краткий особый исход и ссылку на детали.
history with filters/loaded pages → detail → Back сохраняет фильтры и scroll.
Сетка сохраняет масштаб, горизонтальную и вертикальную позицию и рабочую
карточку; после reload данные повторно проверяются сервером. CTA турнира ведёт
сразу в `/tournaments/new`, а `/tournaments` остаётся списком. `/start` приводит
к прямым действиям Home с фокусом. Явный Home из Judge ожидает ответ release
и показывает успех либо неопределённость на Home.
...
Для ошибки построения сетки action-local alert расположен внутри открытого
Dialog; для immersive Judge текст ошибки и подписи коррекции читаемы на тёмном
фоне, а подписи связаны с двумя различными полями.
