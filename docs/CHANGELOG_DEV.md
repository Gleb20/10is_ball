## 2026-09-19 — этап 3: aggregate candidate и коррекция позднего каталога

- Единственный `pnpm run ci` прошёл cleanup 4/4, quality 1188/1188 и PostgreSQL 72/72, но browser 0/60 не стартовал из-за macOS sandbox; команда не повторялась и не считается успешной. Внешний historical browser recovery до последней frontend-коррекции прошёл 60/60 + 9 foundation.
- Deterministic Red подтвердил, что поздний `create-options` мог стереть уже введённый поиск в MatchCreate и MatchDetail. Исправление различает пользовательское редактирование и внешнюю смену выбранного slot: R2 focused 32/32, финальный quality 1193/1193. PostgreSQL-код после 72/72 не менялся.
- Первый полный compiled browser на исправленном дереве прошёл 59/60 + 9 foundation; один desktop GAP-012 timeout ожидания первой option остался без доказанной причины. Финальный разрешённый original-order прогон на свежем disposable PostgreSQL прошёл 60/60 + 9 foundation. Пассивные desktop/mobile captures реального fixture сохранили query, identity и выбор без blur, page error или неожиданного перехода. Зелёный прогон не объясняет более ранний timeout и не доказывает, что тот был harmless, pre-existing или fixed. Адресная матрица с 476 px frequent/recent/team блоком прошла 4/4 + 9 foundation как supporting discrimination.
- Frozen helper имел непригодный `vendorDOMShapeMatches`: он требовал `data-readonly` у редактируемого поля. Raw geometry/event evidence сохранено с этим ограничением. Исторические evidence не удалялись; отклонённая попытка удалить два старых quality-каталога заменена точечной редакцией единственной synthetic loopback URL с provenance hashes.
- Локальный кандидат остаётся 4.1.1 на base `550d3680a08a8faf4e1e4afbfcc373c942f155d0`; публичный стенд остаётся 4.1.0. Commit, push, tag, deploy и публичные мутации не выполнялись. Итоговая приёмка статусов ожидается.

## 2026-09-19 — этап 3 WO4: общий статус без ложного раскрытия

- Принятый [inventory/target](audit/evidence/stage3-wo4/README.md) подтвердил на синтетическом Chromium декоративный caret и собственный pointer/hover у неинтерактивного `StatusChip`; локальные Judge/Team/Profile/bracket обозначения отделены от общего компонента. Через публичный `startIcon={false}` и scoped CSS общий chip теперь показывает прежний текст/tone без caret и собственного hover; курсор наследует карточку-ссылку. Vendor, семь страниц-consumers, Home `winnerSide`, права и API не менялись.
- Chromium на 360/390/1440 проверил все семь прямых consumers: 21 контекст, 108 chip, 108 hover checks, 52 пары сопоставлены с принятым baseline по тексту/tone/высоте, 0 chip tab stops, 0 page errors/overflow. Составные фактические цвета дали минимум 4.90:1; empty/error/pending Home, OS dark preference и immersive Judge оставлены читаемыми. Focused patterns/Home 18/18, web 287/287, typecheck и временная production-сборка прошли. Полный Stage 3 gate, device/AT, доменные consumers и независимое WO4 review впереди; версия/commit/push/deploy не выполнялись.

## 2026-09-19 — этап 3 WO3 R2: конечная версия readback сетки

- Terra R1 REWORK воспроизведён: POST v5 + GET без версии или с `invalid` ошибочно закрывал Dialog, потому что сравнение с `NaN` не срабатывало; старый v4 уже блокировался. Первичный и явный повторный GET теперь требуют конечную версию не ниже подтверждённой. Табличный Red 2 failed / 11 passed, focused Green 13/13, весь web 287/287, typecheck и docs audit 169 файлов прошли; [R2 evidence](audit/evidence/stage3-wo3-r2/README.md). Frozen R1/WO1/WO2 evidence не менялись, WO4 runtime не запускался. Полный Stage 3 gate, независимое review и выпуск впереди.

## 2026-09-19 — этап 3 WO3: payload, Dialog и контраст Judge

- На принятой WO2 R2 базе поля отправленного матча отключаются до ответа, ручной picker сохраняет цель включения, а Judge setup фиксирует выбранные server/side и подтверждённый start между шагами. Известный отказ setup позволяет явный новый setup без нового start; неизвестный исход start/setup не повторяется после GET.
- Ошибка построения сетки остаётся внутри открытого Dialog; известный исправимый отказ и неизвестный исход разделены. Unknown переживает Close/reopen на той же странице, Confirm остаётся недоступным после раннего/failed GET. Успех показывается только после подтверждённого ответа и актуального чтения. Pending Close policy и backend receipt не менялись.
- Scoped dark Judge error и две native correction labels получили контраст 7.50:1/15.47:1 в production-preview Chromium на 360/390/1440. Браузерный Red показал фокус на BODY после Escape из окна сетки; scoped возврат к фактической кнопке запуска дал Green на всех трёх ширинах, общий Dialog не менялся. Подтверждённый POST с неудачным readback остаётся заблокированным до явного успешного GET, после которого окно закрывается; unknown POST от GET не разблокируется. Набор Red/Green и ограничения — [evidence](audit/evidence/stage3-wo3/README.md). Focused 66/66, web 284/284, typecheck и build прошли; проверка ручного состава использует mocked synthetic store, не PostgreSQL. Предсуществующий dev StrictMode replay занесён как открытый BUG-041 без вывода о production-дефекте. Независимое review, полный Stage 3 gate и выпуск впереди.

## 2026-09-19 — этап 3 WO2 R2: повторная ошибка и размер текста auth-полей

- Terra R1 REWORK воспроизведён: при повторной отправке неизменённого несовпадения React объединял `error=null` и тот же текст, поэтому `useEffect[error]` не возвращал фокус Alert. R2 задаёт новый focus revision для каждой явной ошибки и фокусирует Alert после commit; редактирование не создаёт нового запроса фокуса. Один Alert, связи обоих полей, значения и те же DOM input сохранены.
- Отдельный Chromium-замер всех четырёх auth input на 360 px подтвердил 14 px у Login Email против 16 px у трёх паролей. Минимум 16 px применён только к AuthLayout input. [R2 evidence](audit/evidence/stage3-wo2-r2/README.md) содержит Red/Green JSON/кадры и оговаривает, что прежнее измерение только пароля было недостаточным. Физическая причина iPhone zoom остаётся непроверенной.
- Focused 20/20, весь web suite 272/272 (42 файла), typecheck, build, docs audit 167 файлов без ошибок и Chromium повторного mismatch/font Green прошли; независимое R2 review ещё впереди. WO1/WO2 R1 evidence не менялись. API/logout/session semantics, версия и публикация не менялись.

## 2026-09-19 — этап 3 WO2: вход и первый пароль, локальный кандидат

- После принятого WO1 R2 исправлен BUG-027: «Выйти» на обязательной смене пароля завершает текущую ограниченную серверную сессию, принимает `401` как уже завершённую, очищает форму/auth state и заменяет route на Login. Save/Logout и повторные нажатия делят один guard; известный отказ сохраняет ввод и требует явного повтора. Серверный API, политика и другие сессии не изменены.
- Для BUG-028/AUTH-001/002/004 поля и основные действия выровнены, раскрытие пароля встроено в каждое поле с `aria-pressed`/`aria-controls`, значение и DOM input сохраняются. Именованный Alert получает фокус после submit error; mismatch связан с обоими полями, policy — с новым паролем, известные причины переведены, неверные credentials остаются общим сообщением. Высокая auth-карточка остаётся достижимой при CSS zoom 200%; глобальная тема не менялась.
- Meaningful browser Red на старом FirstPasswordPage: 0 logout POST, прежний `/auth/me` 200 после Exit/Back. Green на in-memory PGlite: 1 POST, текущий `/me` 401, другая сессия 200 и Back→Login. Focused tests 18/18; весь web suite 270/270, typecheck и build прошли. Chromium matrix: 10 форм-состояний 360/390/1440/200% CSS zoom/OS dark preference, 2 held-request сценария с сохранением значений и фокуса; [evidence](audit/evidence/stage3-wo2/README.md) и raw logs. Физический iPhone, OS autofill, WebKit, VisualViewport/keyboard/safe area и spoken AT не проверены; BUG-040 не закрыт. Полный Stage 3 CI и независимое WO2 review ожидаются. Commit/push/version/deploy не выполнялись.

## 2026-09-19 — этап 3 WO1 R2: исправление тесной области списка

- После независимого Terra `REWORK` воспроизведена ветка `max(above,below)<60` на реальном Autocomplete: clip 120 px, по 30 px над/под полем, menu `top:0`, 20-я option за границей и повторный `scrollIntoView` при ручной прокрутке. R2 заново измеряет clip после одной попытки прокрутки и при невозможности разместить список внутри короткого контейнера использует доступную область viewport. В page и вложенном Dialog Green активная option доступна по hit test, `scrollIntoView` не вызывается, Enter выбирает `u20` с сохранением фокуса. [Red/Green и replay](audit/evidence/stage3-wo1-r2/README.md).
- Повторно прошли page 360/390/1280 и 390×500 height approximation, Dialog 360/390/1280: option visible, zero-result status, Enter/focus и ручная прокрутка. Представительные field+error, selected picker и radio-card проверены на 360 px и при 200% CSS zoom с 720 px viewport (360 effective CSS px), без horizontal overflow; длинная radio-card при zoom требует внутренней прокрутки. CSS zoom не выдаётся за браузерный zoom или физический телефон. Focused ui.a11y 10/10, весь web suite 261/261, web typecheck и build прошли; логи и хеши — во внешнем R2 receipt. Совокупный CI/PostgreSQL gate и независимый review R2 ещё не выполнены.
- Уточнение R1: mixed-theme эксперимент с тёмными токенами не доказывает ни производственный дефект, ни причину в прежней библиотеке. Приложение фиксирует light theme; фактическая immersive оболочка Judge проверена отдельно. Глобальная тема не менялась.

## 2026-09-18 — этап 3 WO1: локальный кандидат shared selectors

- На базе `550d3680` исправлены BUG-018/019/020/023/025 и AUTH-004/a01: устойчивые option IDREF и DOM focus при Enter, закрытие popup при Tab/Escape/disabled, token-поиск имени, размещение списка с учётом viewport и прокручиваемого диалога, один внешний focus ring, явные loading/ready/error/retry в UserPicker и judge handover. Существующие ID/exclusions, кнопки и payload не менялись. Vendored `ic-kit.js` изменён адресно: его приватные filter/listbox/placement недоступны через app adapter; массовой перегенерации нет.
- Fresh Red: option IDREF, remount MatchCreate, reversed-name search, pending/error picker, native handover disabled и Chromium rects за viewport/границу Dialog; BUG-020 computed inner square outline и selected shadow. После исправления web tests 261/261, web typecheck и production build прошли. Browser local harness: 360/390/desktop geometry, mobile-height resize, ручная прокрутка без ловушки, открытый список с длинными именами в Dialog, турнирный/командный picker pending→503→один GET retry→ID `u2`, judge handover 844×390/desktop в фактическом immersive shell, light state matrix и dark-token diagnostic. [Evidence](audit/evidence/stage3-wo1/geometry.json). Старые dark кадры на белом фоне не подтверждали контраст; на dark-token surface подписи общего поля/checkbox и текст outlined button дали 1.31/1.46/2.11:1. В приложении фиксирована light theme, этот эксперимент не выдаётся за проверку production dark. Полный CI/PostgreSQL gate отложен до интеграции этапа; независимое review WO1 и `verified_local` ещё не заявлены. WebKit/physical keyboard/safe area/spoken AT не проверялись.

## 2026-09-18 — D36, этап 2: терминальный release gate

- Corrective test-only SHA `550d3680a08a8faf4e1e4afbfcc373c942f155d0` опубликован в `main`; hosted CI `35388043867` завершил Quality, PostgreSQL integration, Compiled browser и Release gate со статусом `success`. Read-only public smoke подтвердил 4.1.0 и exact SHA на web/API/proxy за 19 попыток/96188 ms, Render `/ready` — `database=ok`. [Терминальный receipt](audit/evidence/stage2-terminal-release-receipt.json). Предшествующий failed run ниже сохранён без переписывания; публичные мутации и ручной remote seed не проводились.

## 2026-09-18 — D36, этап 2: выпуск 4.1.0

- Application commit `0279657b227bd6ca10116376bec0e2e11561ab4b` fast-forward опубликован в `main` через native Render/Vercel Git integrations. Read-only `pnpm run smoke:public` подтвердил web/API/proxy exact SHA и 4.1.0 за 14 попыток/70276 ms; отдельный smoke проверил 7 endpoints, Render `/ready` вернул `database=ok`. [Публичный receipt](audit/evidence/stage2-public.json). Мутации публичных данных, ручные remote migrations и seed не выполнялись.
- Публичная release parity не повышает локально принятые 23 атома до `verified_prod`: GAP-015 остаётся `verified_local`, GAP-030/031 — `in_progress`. Browser Back из Judge, HOME-003 Maps/iPhone, физический iPhone, WebKit, spoken AT и пользовательская приёмка остаются открытыми.
- Первый hosted CI `35385224009` завершился `failure`: Quality/PostgreSQL success, compiled browser 58/60 и Release gate failure. На пустых уведомлениях глобальный локатор теста AT-HOME-003 совпал с двумя корректными кнопками «На главную». [Correction receipt](audit/evidence/stage2-ci-correction.json): детерминированный Red 67/69 и scoped test-only Green 69/69 на отдельных disposable PostgreSQL. Corrective commit и повторный hosted CI ждут review; успешный релизный gate пока не заявлен.

## 2026-09-18 — D36, этап 2: локальная приёмка R2 v2

- Реализация GAP-015 и принятых атомов GAP-030/031 локально принята координатором после независимого Terra PASS, проверки visual/scope и реального forward/rollback пакета. [Итоговый receipt](audits/2026-09-13-ux-ui/implementation/stage2-final-evidence/stage2-final-receipt.json) связывает base `e3b22876`, frozen manifest `54e922b791dcbdb4c0594a691f108cf48c82627c68c1ccc37c4fc0081d9bdf8c`, один завершённый `pnpm run ci` **1291/1291** (cleanup 4, quality 1146, PostgreSQL 72, compiled browser 69) без failed/skipped/todo/interrupted и 74/74 source hashes после CI. В docs скопированы lane summaries, Terra/root receipts; 23 user atoms этапа 2 получили evidence-bound `verified_local`, HOME-004/a01 связан с AT-HOME-001 и bounded tournament projection. Исторический GAP-015 остаётся superseded; HOME-003 Maps/iPhone не закрыт.
- GAP-030 остаётся `in_progress`: явный Home из Judge освобождает слот, но native Browser Back оставляет его активным; безопасная навигация/восстановление — этап 6. GAP-031 остаётся `in_progress` только для невоспроизведённой HOME-003 гипотезы; реализованная часть проверена. Физический iPhone, WebKit, spoken AT и пользовательская приёмка не проводились. Кандидат 4.1.0 не закоммичен и не опубликован; public stand остаётся на 4.0.0.

## 2026-09-18 — D36, этап 2 R2: исправление замечаний к R1

- При первом ожидании/ошибке Home история и рейтинг остаются доступны вместе с остальными разрешёнными переходами; неизвестные показатели не заменяются нулями. Турнирные сводки используют строки игр, участников и действующих судейских сессий без `getMatch` каждой игры сетки; чужой активный admin каталог не становится личными текущими делами. Обычная завершённая карточка матча показывает стороны и счёт первыми, длительность третьей, кубок у подтверждённой сервером стороны; повтор имени победителя и обычный terminal chip убраны, особые исходы сохранены.
- Meaningful Red: Home component 3 failures (pending/error входы, отметка победителя), API Home 1 failure (16 лишних `getMatch` в крупной чужой сетке). Focused Green: Home component 10/10, API Home 9/9, API/web typecheck. Compiled R2 browser lane после первых исправлений: 60/60 + 9 foundation; визуальный кадр 390 выявил отрыв кубка от длинного имени. После привязки кубка и исправления независимости browser fixture от onboarding узкий compiled HOME-007 прошёл 2/2 + 9 foundation на чистой БД; desktop/390 кадры сохранены отдельно от R1. HOME-004/a03 согласован с D36 как player-only; HOME-007/a03 target уточнён, 236 coverage rows прошли checker. Финальный `pnpm run ci` на итоговом дереве и независимое review ещё впереди; R1 пакет сохранён отдельно. Browser Back, Maps и physical iPhone не объявлены исправленными.
- Дополнительный Red при freeze review: чужой активный турнир в legacy `activeEvents.tournament` терял существующий `topThree`, хотя не должен становиться личным `currentTasks`. Для одного выбранного события восстановлена прежняя сводка через пакетную проекцию без full-detail `getMatch` каждой игры; regression проверяет имя победителя, `currentTasks=[]` и ровно один `tournaments.get` на большой чужой сетке. После исправления API Home 9/9 и typecheck прошли; финальный freeze v2 заменяет для ревью исходный R2, который сохранён неизменным.

## 2026-09-18 — D36, этап 2: локальный кандидат 4.1.0 для ревью
- GAP-030/031 и текущий target GAP-015: Home показывает два прямых действия, все собственные текущие дела по роли, пять последних событий с серверным фильтром «Только мои» и top-3. Пять глобальных вкладок убраны; старый `/start` ведёт к действиям с фокусом, `/tournaments/new` сразу открывает создание. История и сетка восстанавливают контекст после повторного чтения сервера; явный выход из Judge на Home подтверждает освобождение слота. API сохраняет совместимый `activeEvents` и добавляет `currentTasks`/`recentRole`, без миграции и расширения прав.
- Проверки на Node 24.20.0: финальный `verify:fast` **1141/1141**; PostgreSQL **72/72** в предшествующем полном прогоне после API-изменений; финальный compiled Chromium desktop/390 **58/58** + migration foundation **9/9**, всего `verify:e2e` **67/67**. Отдельный полный `pnpm run ci` не завершился успешно: его browser lane остановился на тестовой fixture, создавшей занятого игрока для следующих турниров; fixture исправлена, затем весь browser lane прошёл. Фокусные Home/History/Bracket 18/18 и web typecheck прошли после последних UI-правок. [Локальный receipt](audits/2026-09-13-ux-ui/implementation/stage2-evidence/stage2-local-receipt.json) разделяет lanes и ограничения.
- Chromium показал остаточный край: Browser Back из Judge оставляет `activeJudge` активным; явный Home освобождает слот. Синтетические Home screenshots для populated/expanded/long-name/empty/error отделены от живых desktop/mobile кадров; physical device/WebKit/spoken AT и пользовательская приёмка не проводились. Кандидат 4.1.0 не закоммичен, не опубликован и ожидает независимого ревью.
