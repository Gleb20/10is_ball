...
it("GAP-029 shows only a fresh team invitation, dismisses it and skips hidden/expired",async()=>{
 mock.notifications.mockResolvedValue({notifications:[{id:"expired",title:"Истекло",type:"team_invitation",actionable:true,expiresAt:"2000-01-01T00:00:00Z"},{id:"read",title:"Прочитано",type:"team_invitation",actionable:true,readAt:future(),expiresAt:future()},{id:"hidden",title:"Скрытый матч",body:"В матч",type:"match_invitation",actionable:true,readAt:null,expiresAt:future()},{id:"one",title:"Первое приглашение",body:"В команду",type:"team_invitation",actionable:true,readAt:null,expiresAt:future()},{id:"two",title:"Скрытый судья",type:"judge_invitation",actionable:true,readAt:null,expiresAt:future()}]});
 render(<MemoryRouter><InvitationNotice userId="u" enabled /></MemoryRouter>);
 expect(await screen.findByText("Первое приглашение")).toBeVisible();expect(screen.queryByText("Скрытый матч")).not.toBeInTheDocument();
...
it("GAP-008 a shown notice disappears at its exact expiration",async()=>{
 vi.useFakeTimers();const expiry=new Date(Date.now()+1000).toISOString();mock.notifications.mockResolvedValue({notifications:[{id:"x",title:"Короткое",type:"team_invitation",actionable:true,expiresAt:expiry}]});
...
it("GAP-008 notice survives StrictMode effect replay",async()=>{
 mock.notifications.mockResolvedValue({notifications:[{id:"strict",title:"Новое в строгом режиме",type:"team_invitation",actionable:true,expiresAt:future()}]});
...
it("GAP-008 previous actor dismiss response cannot navigate the new actor",async()=>{
 ...
 mock.notifications.mockResolvedValueOnce({notifications:[{id:"a",title:"Приглашение A",type:"team_invitation",actionable:true,expiresAt:future()}]}).mockResolvedValue({notifications:[]});
