...
## 4.1.1 — локальный кандидат 2026-09-19

- Выбор игроков сохраняет видимую активную опцию и уже введённый поиск при поздней загрузке каталога, принимает имя и фамилию в любом порядке и различает загрузку, пустой результат и ошибку.
- Формы входа и первого пароля получили понятные ошибки, устойчивый фокус, независимое раскрытие паролей и корректный выход из текущей сессии.
- Диалоги и настройки матча сохраняют отправленные значения во время запроса, а ошибки сетки и судейства остаются читаемыми и не скрывают неопределённый результат.
- Общие неинтерактивные статусные метки больше не показывают ложный caret и собственный hover; остальные доменные метки и физические проверки устройства остаются отдельными задачами.

## 4.1.0 — кандидат 2026-09-18

- Главная стала единым входом: быстрый запуск матча и турнира, текущие дела по роли, история и рейтинг без постоянных вкладок и промежуточного экрана «Начать».
- Контекстный возврат сохраняет историю и положение турнирной сетки; переход на Главную из судейства проходит через освобождение слота и сообщает неопределённый исход.
- История на Главной умеет показывать последние пять событий с участием игрока, сохраняя доступность и прежние права просмотра.
- При ошибке загрузки Главной история и рейтинг остаются доступными; завершённый матч в краткой карточке выделяет победившую сторону и длительность без лишней строки статуса.
