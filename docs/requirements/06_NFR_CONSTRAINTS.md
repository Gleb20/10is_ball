...
Исключение для текущего бесплатного Render: пробуждение sleeping service может
занять до 60 секунд только при явном UI-состоянии «сервис просыпается/загрузка»,
ограниченном timeout и доступном Retry. После пробуждения запросы оцениваются по
обычным SLO выше; cold-start допуск не маскирует warm latency/error (ADR D21).
...
- HTTP responses и логи не содержат password/session hashes, открытые credentials
  или внутренние auth persistence-поля.
...
## 5.1. Время и календарь

- абсолютные timestamps хранятся и передаются в UTC;
- пользовательское отображение времени — `Europe/Moscow`;
- границы календарного дня, недели и месяца для рейтинга/фильтров считаются в
  `Europe/Moscow`, независимо от timezone браузера или хоста (ADR D20).
...
Текущий Neon Free public stand по D31 является disposable и не считается
выполнением требований выше. Пока на нём нет ценных данных, отсутствие
production-grade backup принято как временный риск и не блокирует delivery.
...
Delivery gate закреплён D31:

- Node `24.20.0`, pnpm `9.15.0`, PostgreSQL `16.15` и lockfile-managed Chromium;
- PR проходит `quality`, `postgres-integration` и `browser-prodlike`; агрегатор
  принимает только три результата `success`, а опубликованный SHA наблюдается
  вручную запущенным read-only release smoke;
- `pnpm verify:all` воспроизводит полный gate локально без `.env` и production
  secrets и обязан оставить 0 failed/skipped/todo и 0 disposable resources;
- hosted metadata имеет точную структуру
  `{ sha, version, environment, dirty }`, full SHA, `dirty=false` и root version;
- state-changing E2E запрещён на public stand; финальный smoke только read-only.
