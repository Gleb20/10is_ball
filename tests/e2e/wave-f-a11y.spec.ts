...
test('Wave F judge disclosure score and landscape',async({page})=>{
 ...
 await login(page);await page.goto('/matches/new');await page.getByLabel('Название').fill('F judge');await page.getByLabel('Создатель играет',{exact:true}).check();await page.getByRole('group',{name:'Соперник: тип участника',exact:true}).getByRole('button',{name:'Гость',exact:true}).click();await page.getByLabel('Гость (Имя Фамилия)').fill('Гость Проверочный');await page.getByRole('button',{name:'Создать матч',exact:true}).click();await expect(page).toHaveURL(/\/matches\/[0-9a-f-]+$/);id=page.url().split('/').at(-1);
