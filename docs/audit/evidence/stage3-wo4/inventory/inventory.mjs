...
const output = "/private/tmp/tab10-wo4-preliminary";
...
const result = { source: "WO3 R2 frozen source built into private/tmp; entirely synthetic intercepted API", baseManifestSha256: "54860798d7c0744a838e7d1b299a2f44ea34f03c7e6176f7663afe51adc030fd", cases: [], unknownRequests: [] };
let browser;
...
async function inspect(page) {
  return page.evaluate(() => {
    ...
    const chipData = chipRoots.map((element) => {
      ...
      return { text: element.textContent?.trim(), tag: element.tagName, role: element.getAttribute('role'), tabIndex: element.tabIndex, visible: rect.width > 0 && rect.height > 0, cursor: css.cursor, color: css.color, background: css.backgroundColor, width: rect.width, height: rect.height, icon: Boolean(icon), iconAriaHidden: icon?.getAttribute('aria-hidden') ?? null, parentActionTag: parentAction?.tagName ?? null, parentHref: parentAction?.getAttribute('href') ?? null };
    });
...
const scenarios = [
  ...
  { name: "matches", route: "/matches", marker: "Матчи", widths: [390] },
  { name: "match-detail", route: "/matches/m-waiting", marker: "Матч waiting", widths: [360, 390, 1440] },
  { name: "tournaments", route: "/tournaments", marker: "Турниры", widths: [390] },
  ...
  { name: "history", route: "/history", marker: "История", widths: [390] },
  { name: "admin", route: "/admin", marker: "Пользователи", widths: [390] },
...
try {
  ...
  for (const scenario of scenarios) {
    for (const width of scenario.widths) {
      ...
      if (width === 390 && ["home", "match-detail", "admin"].includes(scenario.name)) {
        row.keyboardFocus = [];
